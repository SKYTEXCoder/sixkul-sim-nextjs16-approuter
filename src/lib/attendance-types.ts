...
export const statusColors: Record<AttendanceStatus, { bg: string; text: string; icon: string }> = {
  PRESENT: {
 ...
 */
export function groupByDate(records: AttendanceViewModel[]): AttendanceDateGroup[] {
 ...
 */
export function groupByExtracurricular(records: AttendanceViewModel[]): AttendanceEkskulGroup[] {
  ...
  return Array.from(groups.values()).sort(
    (a, b) => a.extracurricular.name.localeCompare(b.extracurricular.name)
  );
}
